...
# -*- coding: utf-8 -*-
""" 
delayed association task training + ablation analysis (cf Fig 6 in paper)
"""
...
from ingredients.dataset import delayed_association as dataset, load_delayed_association as load_dataset
...
OBSERVE = True #save model results
nepochs = 20
seed = 321 #good fast seed
...
def main(_config, seed):
    ...
    classification = True      
    output_size = _config['dataset']['npoints']
    
    train_window = _config['dataset']['window_size']   
    predict_last = train_window is None
    ...
    model.init_dataset_details(batch_size, seq_len, classification,
                                 predict_last, final_window=train_window)    
    ...
    plt.axhline(0, c='black', linestyle='dotted')
    plt.axvline(ablate_timestep-1, color='black', label='ablation point')
    plt.legend()
