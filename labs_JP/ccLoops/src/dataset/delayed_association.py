...
# -*- coding: utf-8 -*-
"""
delayed association task (see Fig 6 in paper)
"""
...
def get_input_target_pairs(input_D, min_value, max_value, npoints, train_window=None):
    ...
    targets = np.arange(npoints, dtype=int)
    
    if train_window is not None:
        targets = targets[:, None].repeat(train_window, axis=1)
    ...
    if input_D == 1:
        ...
        rand_numbers = rg.choice(nposs_points, size=npoints, replace=False)
...
class BatchGenerator:
    def __init__(
        ...
        random_state=None, npoints=1, final_input=False
    ):
        self.size = size
...
def load_delayed_association(
    ...
    input_vals, target_vals = get_input_target_pairs(input_D, min_value, max_value, npoints,
